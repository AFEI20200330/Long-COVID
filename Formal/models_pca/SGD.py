...
for name, mp in models_params.items():
    ...
    # 根据是否支持predict_proba选择使用decision_function或predict_proba
    if hasattr(best_model, "predict_proba"):
        y_scores = best_model.predict_proba(X_test)[:, 1]  # 用于计算ROC AUC
    else:
    ...
    if hasattr(best_model, "decision_function"):
        ...
        explainer = shap.KernelExplainer(best_model.predict, shap.sample(X_train,25,random_state=42))
    ...
    plt.savefig(f'../Results/PCA/Figures/{name}_shap_summary_plot.jpg', dpi=600)
    plt.show()
    plt.close()
    ...
    plt.savefig(f'../Results/PCA/Figures/{name}_shap_beeswarm_plot.jpg', dpi=600)
    plt.show()
    plt.close()
    ...
    for i in range(min(3, len(X_test))):  # 对前5个测试样本绘制瀑布图
        ...
        # 调整左边距和右边距
        plt.gcf().subplots_adjust(left=0.3, right=0.95)  # 调整右边距以确保内容完整显示
        plt.savefig(f'../Results/PCA/Figures/{name}_shap_waterfall_plot_sample{i}.jpg', dpi=600)
        plt.show()
        plt.close()
