...
from sklearn.metrics import roc_auc_score, make_scorer, confusion_matrix, auc, ConfusionMatrixDisplay, \
    roc_curve
...
models_params = {
    'DecisionTreeClassifier': {
        ...
        'params': {
            ...
            'max_depth': [None, 1, 2, 5, 10, 15, 20, 25, 30],
...
for name, mp in models_params.items():
    ...
    # plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_summary_plot_for_class1.jpg', dpi=600)
    # plt.show()
    # plt.close()
    ...
    # 柱状图
    plt.figure()
    shap.plots.bar(shap_values_class1_, max_display=20, show=False)
    plt.gcf().subplots_adjust(left=0.35, right=0.95)  # 调整右边距以确保内容完整显示
    plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_bar_plot_for_class1.jpg', dpi=600)
    plt.show()
    plt.close()
    ...
    plt.gcf().subplots_adjust(left=0.35, right=0.95)  # 调整右边距以确保内容完整显示
    plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_beeswarm_plot_for_class1.jpg', dpi=600)
    plt.show()
    plt.close()
    ...
    for i in range(min(3, len(X_test))):  # 对前5个测试样本绘制瀑布图
        ...
        plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_waterfall_plot_sample{i}_for_class1.jpg', dpi=600)
        plt.show()
        plt.close()
    ...
    # plt.gcf().subplots_adjust(left=0.35, right=0.95)  # 调整右边距以确保内容完整显示
    # plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_summary_plot_for_class0.jpg', dpi=600)
    # plt.show()
    # plt.close()
    ...
    # 柱状图
    plt.figure()
    shap.plots.bar(shap_values_class0_, max_display=20, show=False)
    plt.gcf().subplots_adjust(left=0.35, right=0.95)  # 调整右边距以确保内容完整显示
    plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_bar_plot_for_class0.jpg', dpi=600)
    plt.show()
    plt.close()
    ...
    plt.gcf().subplots_adjust(left=0.35, right=0.95)  # 调整右边距以确保内容完整显示
    plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_beeswarm_plot_for_class0.jpg', dpi=600)
    plt.show()
    plt.close()
    ...
    for i in range(min(3, len(X_test))):  # 对前5个测试样本绘制瀑布图
        ...
        # 调整左边距和右边距
        plt.gcf().subplots_adjust(left=0.3, right=0.95)  # 调整右边距以确保内容完整显示
        plt.savefig(f'../Results/NoPCA/Figures/{name}_shap_waterfall_plot_sample{i}__for_class0.jpg', dpi=600)
        plt.show()
        plt.close()
