...
for name, mp in models_params.items():
    ...
    for idx, params in enumerate(param_grid):
        ...
        try:
            ...
            print(f"模型 {name} 在参数组合 {params} 时出现错误：{e}")
            continue
    if model_results:
        # 保存模型结果到总的结果列表
        all_results.extend(model_results)
    else:
        print(f"模型 {name} 没有成功的训练结果。")
    print(name)

# 将所有模型的结果保存到一个Excel文件
if all_results:
    all_results_df = pd.DataFrame(all_results)
    all_results_df.to_excel('all_model_performance_precision_without_pca.xlsx', index=False)

print("所有模型的评估结果和每个参数组合的预测结果已保存到Excel文件。")
